...
/*
 * CRC16/MODBUS查表。表内容由原运行期建表逻辑按0xA001多项式生成。
 * 使用const表可避免FreeRTOS下首次调用动态建表的竞态,同时减少运行期初始化开销。
 */
...
uint16_t crc_16(const uint8_t *input_str, uint16_t num_bytes)
{
    ...
    {
        crc = (crc >> 8) ^ crc16_table[(crc ^ (uint16_t)input_str[i]) & 0x00FFU];
    }
...
/*
 * @brief 基于上一轮CRC值增量更新一个字节。
 */
uint16_t update_crc_16(uint16_t crc, uint8_t c)
{
    return (crc >> 8) ^ crc16_table[(crc ^ (uint16_t)c) & 0x00FFU];
}
