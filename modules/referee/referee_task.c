...
#ifndef REFEREE_UI_TEST_ENABLE
#define REFEREE_UI_TEST_ENABLE 0U // 正式工程默认显示application层传入的真实状态,不自动伪造测试状态
#endif
#define REFEREE_UI_INIT_TIMEOUT_MS 5000U // UI任务启动时等待裁判系统ID的最长时间
#define REFEREE_UI_INIT_RETRY_MS 100U    // 裁判系统尚未上线时,UI任务的低频重试周期

static Referee_Interactive_info_t *Interactive_data; // UI绘制需要的机器人状态数据
static referee_info_t *referee_recv_info;            // 接收到的裁判系统数据
static uint8_t referee_ui_ready;                      // UI是否已经完成初始绘制
static uint32_t referee_ui_last_boot_try_tick;        // 上一次尝试初始化UI的tick,避免队列忙时高频重复投递
uint8_t UI_Seq;                                      // 包序号，供整个referee文件使用
// @todo 不应该使用全局变量

/**
 * @brief  判断各种ID，选择客户端ID
 * @param  robot_id 裁判系统下发的本车机器人ID
 * @retval none
 * @attention
 */
static void DeterminRobotID(uint8_t robot_id)
{
    // 蓝方机器人ID从101开始;红方雷达为9,不能简单用>7判断颜色。
    referee_recv_info->referee_id.Robot_Color = robot_id >= RobotID_BHero ? Robot_Blue : Robot_Red;
...
static uint8_t RefereeUIBootDraw(uint8_t robot_id);                       // 首次绘制静态UI和动态UI占位
#if REFEREE_UI_TEST_ENABLE
...
referee_info_t *UITaskInit(UART_HandleTypeDef *referee_usart_handle, Referee_Interactive_info_t *UI_data)
{
    ...
    referee_ui_ready = 0U;
    referee_ui_last_boot_try_tick = 0U;
    UI_Seq = 0U;
...
void UITask()
{
    RefereeTxProcess(); // 先泵一次发送队列,让初始化绘制投递的多帧UI按限速持续发出

    if (referee_recv_info == NULL || Interactive_data == NULL)
        return;

    if (referee_ui_ready == 0U)
    {
        referee_info_t referee_snapshot;
        uint32_t now;

        (void)RefereeGet(&referee_snapshot);
        if (referee_snapshot.GameRobotState.robot_id != 0U)
        {
            now = osKernelGetTickCount();
            if (referee_ui_last_boot_try_tick == 0U ||
                (now - referee_ui_last_boot_try_tick) >= REFEREE_UI_INIT_RETRY_MS)
            {
                referee_ui_last_boot_try_tick = now;
                (void)RefereeUIBootDraw(referee_snapshot.GameRobotState.robot_id);
            }
        }
        else
        {
            // 裁判系统还没上线时低频重试,避免UI任务1ms空转。
            osDelay(REFEREE_UI_INIT_RETRY_MS);
        }
        return;
    }
...
static uint8_t RefereeUIBootDraw(uint8_t robot_id)
{
    if (referee_recv_info == NULL || robot_id == 0U)
        return 0U;

    DeterminRobotID(robot_id);                                    // 确定ui要发送到的目标客户端
    if (UIDelete(&referee_recv_info->referee_id, UI_Data_Del_ALL, 0) != HAL_OK) // 清空UI
        return 0U;
    ...
    if (UIGraphRefresh(&referee_recv_info->referee_id, 2, UI_Energy[1], UI_Energy[2]) != HAL_OK)
        return 0U;
...
uint8_t MyUIInit(void)
{
    ...
    {
        osThreadExit(); // 如果没有初始化裁判系统则直接退出UI任务
        return 0U;
    }

    while (waited_ms < REFEREE_UI_INIT_TIMEOUT_MS)
    {
        (void)RefereeGet(&referee_snapshot);
        if (RefereeUIBootDraw(referee_snapshot.GameRobotState.robot_id) != 0U)
            return 1U;

        osDelay(REFEREE_UI_INIT_RETRY_MS);
        waited_ms += REFEREE_UI_INIT_RETRY_MS;
    }

    LOGWARNING("[ref_task] referee not ready, UI init will retry in task");
    return 0U;
}
...
static void MyUIRefresh(referee_info_t *referee_recv_info, Referee_Interactive_info_t *_Interactive_data)
{
    if (referee_recv_info == NULL || _Interactive_data == NULL)
        return;
