...
`modules/imu` 是云台 IMU 的姿态解算与温控模块。它使用 `modules/BMI088` 获取传感器数据，使用 `modules/algorithm` 中的四元数 EKF 进行姿态融合，并通过 `bsp/pwm` 控制 BMI088 加热 PWM。

本模块不直接操作 BMI088 寄存器，也不直接管理 SPI 片选。BMI088 芯片级读写由 `modules/BMI088` 完成，SPI、GPIO、PWM 等硬件访问由 BSP 层统一封装。
...
这两个切换只影响 `BMI088Acquire()` 的软件读取分支，不会重新配置 BMI088 的 DRDY 寄存器。真正的硬件 DRDY 配置由 `BMI088Register()` 初始化时的 `INS_BMI088_WORK_MODE` 决定。
...
触发模式需要 CubeMX 开启 `EXTI line[15:10] interrupts`，当前建议优先级为 `5,0`。如果关闭 `EXTI15_10_IRQn`，触发模式下 `INS_Task()` 会长期等不到完整帧，最终进入 BMI088 离线保护。

触发模式并不是硬件级严格同步采样。当前逻辑要求 acc 和 gyro 都有未处理 DRDY 后才读取，并在读取前后检查 DRDY 计数，尽量避免发布跨采样窗口的数据。
...
4. 若读取成功：
   ...
   - 更新机体系/导航系基向量和运动加速度。
   - 更新 `Yaw/Pitch/Roll/YawTotalAngle`。
   - 调用 `VisionSetAltitude()` 将姿态同步给视觉发送链路。
