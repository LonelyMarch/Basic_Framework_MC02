...
void INS_Task(void)
{
    ...
    if ((count % 1) == 0)
    {
        ...
        if (BMI088Acquire(ins_bmi088, &ins_bmi088_data) == 0U)
        {
            if (bmi088_has_complete_frame == 0U)
            {
                ...
                if (ins_bmi088_wait_frame_count >= INS_BMI088_OFFLINE_FAIL_COUNT)
                {
                    ...
                    {
                        LOGERROR("[ins] BMI088 offline, disable temperature control, reason=wait trigger frame timeout, count=%lu",
        ...
        else
        {
            ...
            // 核心函数,EKF更新四元数
            IMU_QuaternionEKF_Update(INS.attitude.Gyro[IMU_AXIS_X], INS.attitude.Gyro[IMU_AXIS_Y], INS.attitude.Gyro[IMU_AXIS_Z],
                                     INS.attitude.Accel[IMU_AXIS_X], INS.attitude.Accel[IMU_AXIS_Y], INS.attitude.Accel[IMU_AXIS_Z], dt);
            ...
            VisionSetAltitude(INS.attitude.Yaw, INS.attitude.Pitch, INS.attitude.Roll);
        }
    }
