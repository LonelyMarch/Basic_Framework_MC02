...
static void sbus_to_rc(const uint8_t* sbus_buf)
{
    ...
    next.rc.rocker_l_ = (((sbus_buf[2] >> 6) | (sbus_buf[3] << 2) | (sbus_buf[4] << 10)) & 0x07ff) - RC_CH_VALUE_OFFSET;
    //!< Channel 2
...
{
    if (rc_usart_instance == NULL || rc_daemon_instance == NULL || rc_usart_instance->recv_len !=
        REMOTE_CONTROL_FRAME_SIZE)
        return;
