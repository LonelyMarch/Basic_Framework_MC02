...
static void CANFatalError(void)
{
    Error_Handler();
...
static void CANAddFilter(CANInstance* _instance)
{
    ...
    //按接收ID分配FIFO,奇数rx_id进入FIFO0,偶数rx_id进入FIFO1
...
static void CANStartFDCANService(CANBusResource* bus)
{
    ...
    if (HAL_FDCAN_ConfigGlobalFilter(bus->handle, FDCAN_REJECT, FDCAN_REJECT, FDCAN_REJECT_REMOTE, FDCAN_REJECT_REMOTE)
        != HAL_OK)
    {
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef* hfdcan, uint32_t RxFifo0ITs)
{
    ...
    if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) || (RxFifo0ITs & FDCAN_IT_RX_FIFO0_FULL) || (RxFifo0ITs &
...
void HAL_FDCAN_RxFifo1Callback(FDCAN_HandleTypeDef* hfdcan, uint32_t RxFifo1ITs)
{
    ...
    if ((RxFifo1ITs & FDCAN_IT_RX_FIFO1_NEW_MESSAGE) || (RxFifo1ITs & FDCAN_IT_RX_FIFO1_FULL) || (RxFifo1ITs &
